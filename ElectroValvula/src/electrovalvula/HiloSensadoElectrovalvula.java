...
public class HiloSensadoElectrovalvula extends Thread {
        ...
        hiloHeartbeat = new Thread(() -> {
            try {
                while (prendido && !Thread.currentThread().isInterrupted()) {
                    hiloHeartbeat.sleep(50000); // Cada 10 segundos
    ...
    private void procesarComando(String orden) {
        ...
        if (orden.equalsIgnoreCase("ON")) {
            estadoValvula = true;
            System.out.println("Electroválvula ABIERTA (Fertirrigacion Activa)");
            ...
            estadoValvula = false;
            System.out.println("Electroválvula CERRADA (Fertirrigacion Detenida)");
