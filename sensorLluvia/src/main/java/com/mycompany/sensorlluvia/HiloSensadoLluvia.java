...
public class HiloSensadoLluvia extends Thread {
    ...
    public HiloSensadoLluvia(Socket clienteLluviaEnviar, PrintWriter haciaServer) {
        ...
        try {
            ...
            this.desdeServer = new BufferedReader(new InputStreamReader(this.clienteLluviaEnviar.getInputStream()));
        } catch (IOException e) {
    ...
    private boolean verificarEstadoSocket() {
        try {
            if (clienteLluviaEnviar.isClosed()) {
                System.out.println("Socket cerrado detectado.");
                return true;
            }
            
            if (!clienteLluviaEnviar.isConnected()) {
                System.out.println("Socket no conectado detectado.");
                return true;
            }
            
            if (clienteLluviaEnviar.isInputShutdown() || clienteLluviaEnviar.isOutputShutdown()) {
    ...
    private void cerrarRecursos() {
        try {
            ...
            if (clienteLluviaEnviar != null && !clienteLluviaEnviar.isClosed()) {
                clienteLluviaEnviar.close();
            }
        } catch (IOException e) {
    ...
    private void reconectar() {
        try {
            ...
            SensorLluvia.restablecerConexion();
        } catch (InterruptedException e) {
            Logger.getLogger(SensorLluvia.class.getName()).log(Level.SEVERE, null, e);
        }
    }
    ...
    public void run() {
        try {
            ...
            while (prendido) {
                ...
                System.out.println("La humedad desde el dispositivo es: " + llueve);
                Thread.sleep(5000);
            }
