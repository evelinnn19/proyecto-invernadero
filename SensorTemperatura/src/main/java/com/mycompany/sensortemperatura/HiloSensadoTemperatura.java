...
public class HiloSensadoTemperatura extends Thread {
    ...
    public HiloSensadoTemperatura(Socket clienteTemperaturaEnviar, PrintWriter haciaServer) {
        ...
        try {
            ...
            this.desdeServer = new BufferedReader(new InputStreamReader(this.clienteTemperaturaEnviar.getInputStream()));
        } catch (IOException e) {
    ...
    private boolean verificarEstadoSocket() {
        try {
            if (clienteTemperaturaEnviar.isClosed()) {
                System.out.println("Socket cerrado detectado.");
                return true;
            }
            
            if (!clienteTemperaturaEnviar.isConnected()) {
                System.out.println("Socket no conectado detectado.");
                return true;
            }
            
            if (clienteTemperaturaEnviar.isInputShutdown() || clienteTemperaturaEnviar.isOutputShutdown()) {
    ...
    private void cerrarRecursos() {
        try {
            ...
            if (clienteTemperaturaEnviar != null && !clienteTemperaturaEnviar.isClosed()) {
                clienteTemperaturaEnviar.close();
            }
        } catch (IOException e) {
    ...
    private void reconectar() {
        try {
            ...
            SensorTemperatura.restablecerConexion();
        } catch (InterruptedException e) {
            Logger.getLogger(SensorTemperatura.class.getName()).log(Level.SEVERE, null, e);
        }
    }
    ...
    public void run() {
        try {
            ...
            while (prendido) {
                ...
                System.out.println("La temperatura desde el dispositivo es: " + temperatura);
                Thread.sleep(5000);
            }
